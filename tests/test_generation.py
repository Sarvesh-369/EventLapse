...
def test_bounce_ball_generator():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        sample0 = gen.generate_sample(control_value=0, seed=5, output_dir=Path(tmp_dir))
        assert sample0.control_parameter_value == 0
        assert sample0.exact_answer == "0"
        assert len(sample0.executable_trace["events"]) == 0

        sample3 = gen.generate_sample(control_value=3, seed=7, output_dir=Path(tmp_dir))
...
def test_blinking_generator():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        sample4 = gen.generate_sample(control_value=4, seed=2, output_dir=Path(tmp_dir))
...
def test_state_machine_generator():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        sample2 = gen.generate_sample(control_value=2, seed=1, output_dir=Path(tmp_dir))
