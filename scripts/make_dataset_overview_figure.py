...
def build_dataset_overview_figure(output_path: Path):
    ...
    # Widescreen 18x11.5 layout focusing heavily on frame size & trace legibility with ZERO wasted space
    fig = plt.figure(figsize=(18, 11.5), dpi=300, facecolor="white")
    col_gs = gridspec.GridSpec(1, 3, figure=fig, wspace=0.06, left=0.005, right=0.995, top=0.995, bottom=0.005)

    for c_idx, tinfo in enumerate(tasks_info):
        col_sub = gridspec.GridSpecFromSubplotSpec(3, 1, subplot_spec=col_gs[c_idx],
                                                   height_ratios=[0.35, 6.2, 3.8], hspace=0.01)

        # --- 1. Header Box (Clean Title & Question Only) ---
        ...
        header_patch = FancyBboxPatch((0.0, 0.0), 1.0, 1.0, transform=ax_header.transAxes,
                                      ...
                                      boxstyle=BoxStyle("Round", pad=0.005))
        ax_header.add_patch(header_patch)

        ax_header.text(0.5, 0.65, tinfo["domain"], transform=ax_header.transAxes,
                       fontsize=18, fontweight="bold", color=tinfo["color"], ha="center", va="center")
        ax_header.text(0.5, 0.22, tinfo["question"], transform=ax_header.transAxes,
                       fontsize=12, fontstyle="italic", color="#111111", ha="center", va="center")

        # --- 2. 6 Frames arranged in 3 Rows of 2 Frames Each (Zero gaps!) ---
        ...
        frame_rows_gs = gridspec.GridSpecFromSubplotSpec(3, 2, subplot_spec=col_sub[1], hspace=0.0, wspace=0.02)
        ...
        for k, (ts, img) in enumerate(extracted):
            ...
            ax_f.text(0.04, 0.80, f"t = {ts:.2f}s", transform=ax_f.transAxes,
            ...
            if is_event:
                ax_f.text(0.96, 0.80, f"★ Event #{event_num}", transform=ax_f.transAxes,
                          fontsize=12.5, fontweight="bold", color="white", ha="right",
                          bbox=dict(boxstyle="round,pad=0.25", facecolor="#e63946", edgecolor="none", alpha=0.95))

        # --- 3. Cleaned Executable Trace Box (Much larger font sizes, no MORSE, no ###) ---
        ...
        ax_trace.text(0.04, 0.92, "Executable Trace", transform=ax_trace.transAxes,
                      fontsize=16.0, fontweight="bold", color=tinfo["color"], va="center")
        ...
        for line in tinfo["cot_lines"]:
            ...
            if line.endswith(":"):
                ...
                ax_trace.text(0.04, y_pos, "Final Answer:", transform=ax_trace.transAxes,
                              fontsize=14.5, fontweight="bold", color="#111111", va="center")
                ax_trace.text(0.48, y_pos, "\\boxed{2}", transform=ax_trace.transAxes,
                              fontsize=15.5, fontweight="bold", color="white", va="center", ha="center",
