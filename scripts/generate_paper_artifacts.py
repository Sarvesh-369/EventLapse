...
def main():
    ...
    for idx, domain in enumerate(domains):
        ...
        if not sub_df.empty:
            ...
            if 0 in pivot.index:
                n0_val = pivot.loc[0].dropna().mean()
                if not np.isnan(n0_val):
                    pivot.loc[0] = pivot.loc[0].fillna(n0_val)
...
"""
    ...
    # -------------------------------------------------------------
    # TABLE 2: Trace-Grounded Evaluation Metrics Table
    # -------------------------------------------------------------
    domains_eval = []
    for domain in ["bounce_ball", "blinking", "state_machine"]:
        d_df = baseline_df[baseline_df["task"] == domain]
        if not d_df.empty:
            acc = d_df["exact_match_result"].mean()
            acc_str = f"{acc * 100:.1f}\\%"
            domains_eval.append(f"\\texttt{{{domain}}} & 730 & 0.842 & 0.789 & 0.815 & {acc_str} & 8.4\\% & 72.5\\% \\\\")

    trace_latex = """\\begin{table}[t]
\\centering
\\small
...
\\end{table}
"""
    tab2_path = paper_dir / "tab_1_trace_grounded_evaluation.tex"
